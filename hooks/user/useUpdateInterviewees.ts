...
import { InterviewAllocationByCompanyCordinator } from "@/service/interviewAllocationByCompanyCordinator";

export const useUpdateInterviewees = () => {
  const { mutate: Allocation, isPending } = useMutation({
    mutationFn: async (allocationData: any) => {
      ...
      try {
        const response = await InterviewAllocationByCompanyCordinator(
          allocationData
        );
        return response;
      } catch (error) {
